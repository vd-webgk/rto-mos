...
<?
    ...
    //если у товара было одно предложение, считаем доступность по его данным
    if (!empty($arResult["OFFERS_TMP"][0])) {
        $arItemIDsTmp = COptimus::GetItemsIDs($arResult["OFFERS_TMP"][0]);
        $totalCountTmp = COptimus::GetTotalCount($arResult["OFFERS_TMP"][0]);   
        $arQuantityData = COptimus::GetQuantityArray($totalCountTmp, $arItemIDsTmp["ALL_ITEM_IDS"]);    
    }
