...
if(!$arResult['USER_ID'] && !isset($arParams['GUEST_ACCESS'])):?>
	...
	<div class="guest-block">
		...
		<div class="" id="catalog-subscriber-auth-form" style="<?=$authStyle?>">
		...
		<div class="catalog-subscriber-identification-form">
			...
			<form method="post">
				...
				<button type="submit" class="button medium"><?=Loc::getMessage('CPSL_BUTTON_SUBMIT_CODE')?></button>
			</form>
		</div>
		<div>
			...
			<form method="post">
				...
				<button type="submit" class="button medium"><?=Loc::getMessage('CPSL_BUTTON_SUBMIT_ACCESS')?></button>
			</form>
		</div>
		</div>
	</div>
...
<!--		</div>-->
	...
	<?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) // Simple Product
	{
		...
		<div class="bx_catalog_item_controls">
			<div>
				<a class="button btn-sm btn-primary" href="<?=$arItem['DETAIL_PAGE_URL'];?>">
					<?=GetMessage('CPSL_TPL_MESS_BTN_DETAIL');?>
				</a>
			</div>
			<div>
				<a id="<?=$arItemIDs['SUBSCRIBE_DELETE_LINK'];?>" class="button transparent" href="javascript:void(0)">
